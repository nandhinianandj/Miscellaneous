...
allwords = word_tokenize(data)
words = list(set(allwords))
data_size, vocab_size = len(allwords), len(words)
...
while True:
  ...
  inputs = [word_to_ix.get(ch, None) for ch in data[p:p+seq_length]]
  targets = [word_to_ix.get(ch, None) for ch in data[p+1:p+seq_length+1]]
